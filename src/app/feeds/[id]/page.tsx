...
export default function FeedArticlesPage() {
  ...
  const feedId = Number(params.id); // Get feed ID from URL
  const { logout } = useAuth();

  // Auth check/redirect should be handled by layout or higher-level component

   if (isNaN(feedId)) {
       return <div className="flex items-center justify-center min-h-screen">Invalid Feed ID</div>;
   }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-secondary">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        {/* Pass feedId to ArticleList to filter articles */}
